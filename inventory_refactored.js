...
(function() {
	inventory = {
		...
		bindEvents: function() {
			var self = this;
			$('#add_item').on('click', self.newItem.bind(self));
			$('#inventory').on('click', 'a.delete', self.deleteItem.bind(self));
		},

		newItem: function(e) {
			this.createItem();
			$('#inventory').append(this.createTemplate(this.lastItemId));
		},
		...
		deleteItem: function(e) {
			...
			var itemId = Number($(e.target).prev('input').attr('value') - 1);
